---
title: "Hidden Nexus: Ford → Sam Houston (3° Link)"
teaser: "A 3-step chain linking Ford to Sam Houston"
scores: {"novelty": 7, "evidence_strength": 8, "narrative_potential": 8}
total_score: 23
degrees_of_separation: 3
entities: ["Ford", "Irvin McDowell", "Union", "Sam Houston"]
evidence_type: "mixed"
date_generated: 2026-08-05
---

# Hidden Nexus: Ford → Sam Houston (3° Link)

**Teaser:** A 3-step chain linking Ford to Sam Houston

## Hidden Chain

1. [[Ford]] —[**cooccurrence**]→ [[Irvin McDowell]]
   > Co-occurrence in Wikipedia article covering both entities
   [Link](https://en.wikipedia.org/wiki/Ford)

2. [[Irvin McDowell]] —[**cooccurrence**]→ [[Union]]
   > Co-occurrence in Wikipedia article covering both entities
   [Link](https://en.wikipedia.org/wiki/Ford)

3. [[Union]] —[**cooccurrence**]→ [[Sam Houston]]
   > Co-occurrence in Wikipedia article covering both entities
   [Link](https://en.wikipedia.org/wiki/Ford)

## Source Trail

Hop 1: Ford → Irvin McDowell
  - Edge type: cooccurrence
  - Source: Wikipedia co-occurrence
  - URL: https://en.wikipedia.org/wiki/Ford

Hop 2: Irvin McDowell → Union
  - Edge type: cooccurrence
  - Source: Wikipedia co-occurrence
  - URL: https://en.wikipedia.org/wiki/Ford

Hop 3: Union → Sam Houston
  - Edge type: cooccurrence
  - Source: Wikipedia co-occurrence
  - URL: https://en.wikipedia.org/wiki/Ford


## Inference Layer

This direct chain suggests a documented historical overlap between the two figures. The connection is traceable but buried in secondary sources rather than primary biographies.

## Open Questions

- Why is this connection absent from mainstream historical narratives?
- What do the intermediate figures reveal about networks of influence?
- Are there other parallel chains that remain unexplored?

