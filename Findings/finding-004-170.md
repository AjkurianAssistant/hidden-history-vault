---
title: "Shadow Chain: Eric Foner → State (4° Link)"
teaser: "A 4-step chain linking Eric Foner to State"
scores: {"novelty": 5, "evidence_strength": 6, "narrative_potential": 6}
total_score: 17
degrees_of_separation: 4
entities: ["Eric Foner", "the Republican Party", "Charles Sumner", "Andrew Johnson", "State"]
evidence_type: "mixed"
date_generated: 2026-08-05
---

# Shadow Chain: Eric Foner → State (4° Link)

**Teaser:** A 4-step chain linking Eric Foner to State

## Hidden Chain

1. [[Eric Foner]] —[**cooccurrence**]→ [[the Republican Party]]
   > Co-occurrence in Wikipedia article covering both entities
   [Link](https://en.wikipedia.org/wiki/Eric_Foner)

2. [[the Republican Party]] —[**cooccurrence**]→ [[Charles Sumner]]
   > Co-occurrence in Wikipedia article covering both entities
   [Link](https://en.wikipedia.org/wiki/Eric_Foner)

3. [[Charles Sumner]] —[**cooccurrence**]→ [[Andrew Johnson]]
   > Co-occurrence in Wikipedia article covering both entities
   [Link](https://en.wikipedia.org/wiki/Eric_Foner)

4. [[Andrew Johnson]] —[**cooccurrence**]→ [[State]]
   > Co-occurrence in Wikipedia article covering both entities
   [Link](https://en.wikipedia.org/wiki/Eric_Foner)

## Source Trail

Hop 1: Eric Foner → the Republican Party
  - Edge type: cooccurrence
  - Source: Wikipedia co-occurrence
  - URL: https://en.wikipedia.org/wiki/Eric_Foner

Hop 2: the Republican Party → Charles Sumner
  - Edge type: cooccurrence
  - Source: Wikipedia co-occurrence
  - URL: https://en.wikipedia.org/wiki/Eric_Foner

Hop 3: Charles Sumner → Andrew Johnson
  - Edge type: cooccurrence
  - Source: Wikipedia co-occurrence
  - URL: https://en.wikipedia.org/wiki/Eric_Foner

Hop 4: Andrew Johnson → State
  - Edge type: cooccurrence
  - Source: Wikipedia co-occurrence
  - URL: https://en.wikipedia.org/wiki/Eric_Foner


## Inference Layer

This longer chain requires a careful reading of intermediary relationships. The bridge figure may not be widely recognized, suggesting that their role as an intermediary between these two historical forces has been overlooked or minimized in mainstream accounts.

## Open Questions

- Why is this connection absent from mainstream historical narratives?
- What do the intermediate figures reveal about networks of influence?
- Are there other parallel chains that remain unexplored?

